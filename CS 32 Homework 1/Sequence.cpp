...
{
    if (pos > size() || pos < 0 || pos > DEFAULT_MAX_ITEMS)
        return -1;
...
int Sequence::insert(const ItemType& value)
{
    ...
    if (size() + 1 > DEFAULT_MAX_ITEMS)         //if sequence is full return -1, can't add anything ////DOUBLE CHECK!!
        return -1;
...
void Sequence::swap(Sequence& other)
{
    ...
    for (int k = 0; k < size(); k++){
        m_sequence[k] = temp.m_sequence[k];
    }
